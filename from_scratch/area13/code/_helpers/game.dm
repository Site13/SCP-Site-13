...
/proc/is_on_same_plane_or_site(var/z1, var/z2)
	if(z1 == z2)
		return 1
	if((z1 in using_map.site_levels) &&	(z2 in using_map.site_levels))
		return 1
	return 0

/proc/max_default_z_level()
	var/max_z = 0
	for(var/z in using_map.site_levels)
		max_z = max(z, max_z)
...
/proc/isSiteLevel(var/level)
	return level in using_map.site_levels

/proc/isNotSiteLevel(var/level)
	return !isSiteLevel(level)
...
/proc/get_active_candidates(var/buffer = 1)
	...
	var/list/candidates = list() //List of candidate KEYS to assume control of the new antagonist roles ~Carn
	var/i = 0
